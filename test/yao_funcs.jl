...
a2 = [-32, -32, -32, -32, -32, -16, -16, -16, -16, -16, 0,
...
## Functions definition
  ...
  testfuns = (  x -> sum(abs2, x),
                ...
                x -> begin
                  k = length(x)
                  sum(abs2, x)/1000 - prod(cos.(x./sqrt.(collect(1:k)))) + 1.0
                end,
                  x -> begin
                      k = length(x)
                      y = 1 .+ (x .+ 1)./4
                      π/k *(10*sin(π*y[1])^2 + 
                      sum(map(i-> (y[i]-1)^2*(1+10*sin(π*y[i+1])^2), 1:k-1)) + (y[k]-1)^2) + 
                      sum(map(z -> ifelse(z>10, 100*(z-10)^4, ifelse(z < -10, 100*(-z-10)^4, 0)), x))
                  end,
                x -> begin
                k = length(x)
                0.1 *(sin(π*3*x[1])^2 +
...
bnds = map(i -> reshape(repeat([bnds_testfuns[i]...], 
           inner = nargs_testfuns[i]), nargs_testfuns[i] + (i==17)*2, 2), 1:length(nargs_testfuns))
...
for i in 1:23
    push!(funs, Dict(:fname => names_testfuns[i],
                    :f => testfuns[i],
                    :domain => bnds[i],
                    :solutions => xmin_testfuns[i],
                    :minima => mins_testfuns[i], 
                    :differentiable => differentiable[i]))
end
